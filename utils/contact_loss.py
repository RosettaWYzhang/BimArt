'''
code reference: https://github.com/hassony2/obman_train/blob/master/mano_train/networks/branches/contactloss.py
'''
import numpy as np
import trimesh
import torch
from utils import contact_util
...
def compute_contact_loss(
    ...
    dists = batch_pairwise_dist(hand_verts_pt, obj_verts_pt) # [bs, Nh, Nobj]
    mins12, min12idxs = torch.min(dists, 1) #bs, Nobj #object centric
    # minimum index along dimension 2
    mins21, min21idxs = torch.min(dists, 2) # bs, Nh # hand-centric
    # Get obj triangle positions
    obj_triangles = obj_verts_pt[:, obj_faces] 
    exterior = contact_util.batch_mesh_contains_points(hand_verts_pt, obj_triangles, tol_thresh=1e-9)
    penetr_mask = ~exterior
    results_close = batch_index_select(obj_verts_pt, 1, min21idxs) # 16, 42, 3
    ...
    if contact_zones == "tips":
        ...
        _, contact_zones = contact_util.load_contacts(
